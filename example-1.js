...
const midiIn = document.getElementById("midi-in");
const midiOut = document.getElementById("midi-out");
...
function listInputsAndOutputs(midiAccess) {
  for (let entry of midiAccess.inputs) {
    let input = entry[1];
    let msg = "Input port ➡︎ [type:'" + input.type + "'] id:'" + input.id +
    ...
    midiIn.innerHTML = msg;
  }

  for (var entry of midiAccess.outputs) {
    var output = entry[1];
    let msg = "Output port ⬅︎ [type:'" + output.type + "'] id:'" + output.id +
...
function onMIDIMessage(event) {
  ...
  display.innerHTML = `<p> Event data: ${event.data} </p><p>${str}</p>`;
